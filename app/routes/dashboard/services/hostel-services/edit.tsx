...
export default function EditHostelService() {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const [hostelServiceResponse, servicesResponse, hostelsResponse, schedulesResponse] = await Promise.all([
        servicesService.getHostelService(id),
      ...
      const hostelServiceData = hostelServiceResponse.data;
      setHostelService(hostelServiceData);
      ...
      // Set form data
      setFormData({
        hostel: hostelServiceData.hostel,
        service: hostelServiceData.service,
        schedule: hostelServiceData.schedule,
        is_active: hostelServiceData.is_active,
      });
      
      // Set service search term if service data is available
      if (hostelServiceData.service_data) {
        setServiceSearchTerm(hostelServiceData.service_data.name);
      }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await servicesService.updateHostelService(id, formData);
      console.log('✅ Hostel service updated successfully:', response.data);
      navigate('/dashboard/services/hostel-services');
    } catch (err: any) {
      console.error('❌ Error updating hostel service:', err);
      console.error('📊 Error response:', err.response?.data);
      console.error('🔍 Error status:', err.response?.status);
      console.error('📝 Error details:', err.response?.data?.details);
      
      // More detailed error handling
      let errorMessage = 'Error al actualizar el servicio de albergue';
  ...
  return (
    <div className="px-4 sm:px-0">
      <div className="mb-6 flex items-center gap-4">
        ...
        <div>
          <h1 className="text-3xl font-bold text-white">Editar Servicio de Albergue</h1>
          <p className="text-gray-300 mt-1">Modificar la asignación del servicio al albergue</p>
        </div>
      </div>
        ...
        <div className="card">
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="flex gap-4">
              ...
              >
                {saving ? 'Actualizando...' : 'Actualizar Servicio de Albergue'}
              </Button>
              <Button
                type="button"
                onClick={() => navigate('/dashboard/services/hostel-services')}
